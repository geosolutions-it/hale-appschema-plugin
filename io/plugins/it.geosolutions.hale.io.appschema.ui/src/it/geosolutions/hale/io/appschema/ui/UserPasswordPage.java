...
/**
 * Configuration page for specifying user name and password to access GeoServer
 * REST services.
 * 
...
public class UserPasswordPage extends AbstractConfigurationPage<IOProvider, IOWizard<IOProvider>> {
	...
	public UserPasswordPage() {
		...
		setDescription("Please enter user name and password to access GeoServer REST services");
	}
	...
	public boolean updateConfiguration(IOProvider provider) {
		provider.setParameter(AppSchemaIO.PARAM_USER,
				Value.of((user.getText().isEmpty()) ? (null) : (user.getText())));
		provider.setParameter(AppSchemaIO.PARAM_PASSWORD, Value.of(password.getText()));
	...
	public void loadPreSelection(IOConfiguration conf) {
		...
		user.setText(configs.get(AppSchemaIO.PARAM_USER).getStringRepresentation());
		password.setText(configs.get(AppSchemaIO.PARAM_USER).getStringRepresentation());
	}
}
